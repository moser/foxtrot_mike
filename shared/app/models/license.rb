class License < ActiveRecord::Base
  ...
  def flights
    Flight.includes(:plane, :from, :to, :crew_members).
           where(Flight.arel_table[:departure].gteq(valid_from).
                 and(Flight.arel_table[:departure].lteq(valid_to || 1.day.from_now.to_date))).
           joins(:crew_members).
